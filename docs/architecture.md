...
По умолчанию Channel Scanner запускается как один Python-процесс: Telegram bot polling, APScheduler jobs, scraper, LLM-суммаризация, grounded RAG и доставка дайджестов работают в одном runtime. PostgreSQL хранит канонические посты и бизнес-состояние, локальный Qdrant содержит только поисковые представления RAG. Опциональный BL-22 reliable path запускает scheduler, outbox relay, digest renderer и Telegram delivery как отдельные роли из того же образа; Kafka передаёт только content-free ссылки на durable work. BL-29 позволяет держать те же роли в наблюдаемом shadow mode без передачи им production delivery ownership.
...
flowchart TD
    ...
    assistant --> rag[Grounded RAG]
    rag --> lexical[PostgreSQL lexical search]
    rag --> vector[Qdrant vector search]
    lexical --> fusion[Parent RRF + rerank]
    vector --> fusion
    fusion --> cited[Validated inline citations]
    cited --> assistant

    db -. transactional outbox .-> relay[Kafka outbox relay]
    relay -. content-free events .-> kafka[(Kafka KRaft)]
    kafka -.-> worker[Opt-in digest and delivery workers]
    worker -.-> sender
...
- RAG разделяет канонический источник и поисковые представления: ответ строится только по scope-authorized оригинальным постам с проверяемыми inline-цитатами; Qdrant, каталог и mem0 не являются доказательствами.

## Grounded RAG

- Администратор утверждает публичные каналы и импортирует их канонические посты в PostgreSQL. Фоновая задача строит summary/full/chunk представления, извлекает metadata и индексирует embeddings в `.data/knowledge`.
- Retrieval объединяет PostgreSQL lexical search и Qdrant vector search через parent-level RRF, затем ограниченно rerank-ит до 20 уже авторизованных родительских постов. Ошибка provider/cost cap возвращает базовое ранжирование, а не ломает ответ.
- Модель возвращает структурированные claims и source IDs; каждый source ID валидируется против разрешенного retrieval-набора перед Telegram-safe HTML rendering. При недостаточных доказательствах interactive path возвращает честный отказ.
- RAG имеет раздельный dev/eval benchmark, content-free quality/latency/cost telemetry и scope enforcement до любого внешнего LLM/rerank-вызова. Детали, результаты и ограничения: [`rag-architecture.md`](rag-architecture.md).

## BL-29 Kafka Shadow Operations (Accepted 2026-08-24)

- Intended shadow mode включается явной комбинацией `KAFKA_ENABLED=1`, `RELIABLE_DIGEST_ENABLED=0`, `MEMORY_ENABLED=1` и запуском `docker compose --profile bl22 up -d`. Профиль запускает broker и четыре роли; он не переписывает app-side `KAFKA_ENABLED`, поэтому включать этот флаг без доступного broker не следует.
...
- BL-29 закрыт и принят только как shadow operations. Это не разрешение на reliable rollout: `RELIABLE_DIGEST_ENABLED=0`, legacy delivery остаётся единственным production path. Ежедневные проверки и безопасный rollback находятся во внутреннем runbook `.planning/kafka-shadow-operations.md`, который намеренно исключен из публичной документации.
...
- `./scripts/run-bl22-stage7-e2e.sh` является отдельным opt-in real-Telegram runner. Он fail-hard проверяет `.env`, строит уникальный image/project, мигрирует чистую PostgreSQL 16 до тогдашнего head `0024`, поднимает Kafka 3.9.2 и не использует production DB/Kafka или существующие shadow roles.
