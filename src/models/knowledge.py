...
class KnowledgeChannel(Base):
    ...
    description_source_hash: Mapped[str | None] = mapped_column(String(64))
    description_updated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class KnowledgeQuery(Base):
    ...
    rag_variant: Mapped[str] = mapped_column(String(64), nullable=False, default="baseline", server_default="baseline")
    rerank_fallback_reason: Mapped[str | None] = mapped_column(String(64))
    rerank_cost: Mapped[float | None] = mapped_column(Numeric(14, 6))
    catalog_selection_ms: Mapped[int | None] = mapped_column(Integer)
    vector_retrieval_ms: Mapped[int | None] = mapped_column(Integer)
    lexical_retrieval_ms: Mapped[int | None] = mapped_column(Integer)
    rerank_ms: Mapped[int | None] = mapped_column(Integer)
    answer_generation_ms: Mapped[int | None] = mapped_column(Integer)
    rendering_ms: Mapped[int | None] = mapped_column(Integer)
...
class RagSearchConfiguration(Base):
    """Immutable, content-free snapshot of an operator-selected RAG variant."""

    __tablename__ = "rag_search_configurations"
    ...
    query_instruction_hash: Mapped[str] = mapped_column(String(64), nullable=False)
...
class KnowledgeEvaluationRun(Base):
    ...
    precision_at_k: Mapped[float | None] = mapped_column(nullable=True)
    question_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, server_default="0")
    labels_complete: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="0")
    configuration_id: Mapped[str] = mapped_column(String(64), nullable=False, default="baseline", server_default="baseline")
    reranker_model: Mapped[str | None] = mapped_column(String(255))
    rerank_fallback_share: Mapped[float | None] = mapped_column(nullable=True)
    correct_abstention_share: Mapped[float | None] = mapped_column(nullable=True)
    false_attribution_share: Mapped[float | None] = mapped_column(nullable=True)
    source_sufficiency_share: Mapped[float | None] = mapped_column(nullable=True)
    claim_coverage_sufficiency_share: Mapped[float | None] = mapped_column(nullable=True)
    faithfulness: Mapped[float | None] = mapped_column(nullable=True)
    citation_validity: Mapped[float | None] = mapped_column(nullable=True)
    citation_completeness: Mapped[float | None] = mapped_column(nullable=True)
    answer_relevance: Mapped[float | None] = mapped_column(nullable=True)
    answer_audit_sample_size: Mapped[int | None] = mapped_column(Integer)
    judge_version: Mapped[str | None] = mapped_column(String(128))
    ...
    p50_retrieval_latency_ms: Mapped[int | None] = mapped_column(Integer)
    p95_retrieval_latency_ms: Mapped[int | None] = mapped_column(Integer)
    p99_retrieval_latency_ms: Mapped[int | None] = mapped_column(Integer)
    retrieval_latency_ms: Mapped[int | None] = mapped_column(Integer)
    p50_answer_generation_ms: Mapped[int | None] = mapped_column(Integer)
    p95_answer_generation_ms: Mapped[int | None] = mapped_column(Integer)
    p99_answer_generation_ms: Mapped[int | None] = mapped_column(Integer)
    answer_generation_ms: Mapped[int | None] = mapped_column(Integer)
    ...
    citation_recall: Mapped[float | None] = mapped_column(nullable=True)
    citation_f1: Mapped[float | None] = mapped_column(nullable=True)
    citation_placement: Mapped[float | None] = mapped_column(nullable=True)
    claim_precision: Mapped[float | None] = mapped_column(nullable=True)
    claim_recall: Mapped[float | None] = mapped_column(nullable=True)
    claim_f1: Mapped[float | None] = mapped_column(nullable=True)
    judge_claim_precision: Mapped[float | None] = mapped_column(nullable=True)
    judge_claim_recall: Mapped[float | None] = mapped_column(nullable=True)
    judge_claim_f1: Mapped[float | None] = mapped_column(nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
